...
public class ConstructorResolver {
    ...
    public Object autowireConstructor(BeanDefinition beanDefinition) {
        ...
        BeanDefinitionValueResolver beanDefinitionValueResolver = new BeanDefinitionValueResolver(beanFactory);
        ConstructorArgument constructorArguments = beanDefinition.getConstructorArgument();
        SimpleTypeConverter simpleTypeConverter = new SimpleTypeConverter();

        for (int i = 0; i < candidates.length; i++) {
            Class<?>[] parameterTypes = candidates[i].getParameterTypes();
            if (parameterTypes.length != constructorArguments.getArgumentCount()) {
                continue;
            }
            ...
            boolean result = valuesMatchTypes(parameterTypes,
                    constructorArguments.getArgumentValues(),
                    argsToUse,
